...
export async function sendRecoveryCode(req: Request, res: Response): Promise<Response> {
	...
	if (!user.result || Object.keys(user.result).length === 0) return res.status(404).json({ message: "User not found" });
...
export async function recoveryCode(req: Request, res: Response): Promise<Response> {
	...
	try {
		...
		if (!user.result || Object.keys(user.result).length === 0) return res.status(404).send({ message: "User not found" });
...
export async function recoveryPassword(req: Request, res: Response): Promise<Response> {
	...
	if (keys[user.result._id].key !== code) return res.status(401).json({ message: "Invalid code" });

	const hashedPassword = await bcrypt.hash(password, 10);
	const updated = await updateOne("users", { _id: user.result._id }, { password: hashedPassword });
